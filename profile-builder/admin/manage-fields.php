...
function wppb_populate_manage_fields(){
	...
	$fields = apply_filters( 'wppb_manage_fields', array(
        ...
        array( 'type' => 'text', 'slug' => 'avatar-size', 'title' => __( 'Avatar Size', 'profile-builder' ), 'default' => 100, 'description' => __( "Enter a value (between 20 and 800) for the size of the 'Avatar'<br/>If not specified, defaults to 100", 'profile-builder' ) ),
        ...
        array( 'type' => 'checkbox', 'slug' => 'turnstile-wp-forms', 'title' => __( 'Display on default WP forms', 'profile-builder' ), 'options' => array( '%'.__('Default WP Login', 'profile-builder').'%'.'default_wp_login', '%'.__('Default WP Register', 'profile-builder').'%'.'default_wp_register', '%'.__('Default WP Recover Password', 'profile-builder').'%'.'default_wp_recover_password', '%'.__('Default WP Comments', 'profile-builder').'%'.'default_wp_comments'), 'default' => 'default_wp_register', 'description' => __( "Select on which default WP forms to display Turnstile", 'profile-builder' ) ),
	...
	if ( $meta_name == 'wppb_manage_fields' ){
		...
		if ( $posted_values['field'] == 'Avatar' ){
			if ( is_numeric( $posted_values['avatar-size'] ) ){
				if ( ( $posted_values['avatar-size'] < 20 ) || ( $posted_values['avatar-size'] > 800 ) )
					$message .= __( "The entered avatar size is not between 20 and 800\n", 'profile-builder' );
...
function wppb_get_field_property_mapping() {
	...
	$mapping = array(
		...
		'Turnstile'          => array('theme', 'turnstile-site-key', 'turnstile-secret-key', 'turnstile-pb-forms', 'turnstile-wp-forms'),
