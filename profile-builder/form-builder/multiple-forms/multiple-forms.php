...
// NOTE: this file mirrors add-ons/multiple-forms/multiple-forms.php, differing
// only in using __DIR__ in place of the WPPB_PAID_PLUGIN_DIR path — kept close
// to that file so upstream changes port easily. Behavior the form-builder needs
// on top (always-on filter registration, editor-chrome cleanup) lives in
// form-builder-side files.

$wppb_module_settings = get_option( 'wppb_module_settings', 'not_found' );
if ( $wppb_module_settings != 'not_found' ){
	if ( isset( $wppb_module_settings['wppb_multipleEditProfileForms'] ) && ( $wppb_module_settings['wppb_multipleEditProfileForms'] == 'show' ) && file_exists( __DIR__ . '/edit-profile-forms.php' ) )
		include_once( __DIR__ . '/edit-profile-forms.php' );

	if ( isset( $wppb_module_settings['wppb_multipleRegistrationForms'] ) && ( $wppb_module_settings['wppb_multipleRegistrationForms'] == 'show' ) && file_exists( __DIR__ . '/register-forms.php' ) )
		include_once( __DIR__ . '/register-forms.php' );
    ...
    if( isset( $args['ID'], $meta_name ) ) {
        ...
        if (!empty($this_forms_fields)) {
            ...
            foreach ($this_forms_fields as $this_forms_field) {
                if ( is_array( $this_forms_field ) && isset( $this_forms_field['id'] ) )
...
function wppb_add_prepopulated_default_fields( $post_id, $post ){
	// When the form-builder (Gutenberg) editor is active for this CPT,
	// the mandatory-field seeding is owned by wppb_fb_seed_new_form_mandatory_fields()
	// in form-builder-defaults.php. Running this legacy full-list prepopulation too
	// would append a SECOND (non-unique add_post_meta) wppb_rf_fields/wppb_epf_fields
	// row, masked by get_post_meta(..., true). Skip it in form-builder mode; classic
	// mode keeps the legacy behavior.
	if ( function_exists( 'wppb_fb_is_active_for' ) && wppb_fb_is_active_for( $post->post_type ) ) {
		return;
	}
...
function wppb_rf_epf_delete_all_fields_callback(){
    ...
    // Only form editors may bulk-delete a form's fields. The nonce alone is
    // surfaced only on the manage-fields admin screen (manage_options), but a
    // valid-nonce request from any authenticated user should still be refused.
    if( ! current_user_can( 'manage_options' ) )
        die('');
